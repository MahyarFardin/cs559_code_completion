...
def load_tokenized_file(file_path: str) -> List[List[str]]:
...
def create_line_level_dataset(sequences: List[List[str]], 
                              ...
                              max_prefix_ratio: float = 0.8) -> List[Tuple[str, str, str]]:
    """
    ...
    for tokens in sequences:
        ...
        for line_idx, line in enumerate(lines):
            ...
            # Prefix length: random between min_prefix_length and max_prefix_ratio of line length
            max_prefix_len = max(min_prefix_length, int(len(line) * max_prefix_ratio))
            min_prefix_len = min(min_prefix_length, len(line) - 1)
            
            # Try multiple split points for diversity
            for split_point in range(min_prefix_len, min(max_prefix_len + 1, len(line))):
...
def main():
    ...
    for split in ["train", "dev", "test"]:
        ...
        print(f"\nProcessing {split}...")
        sequences = load_tokenized_file(input_file)
        ...
        if args.token_level:
            ...
            token_examples = create_token_level_dataset(sequences, args.max_length)
        ...
        if args.line_level:
            ...
            line_examples = create_line_level_dataset(
                sequences, 
